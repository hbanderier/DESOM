...
class PerfLogger:
    ...
    def log(self, summary, verbose=0):
        ...
        if verbose > 0:
            ...
            if verbose == 1:
                print(
                    "[Train] - Lr={:f}, Lsom={:f}, L={:f}".format(
                        results["Lr"], results["Lsom"], results["L"]
                    )
                )
                if self.with_validation:
                    print(
                        "[Val] - Lr={:f}, Lsom={:f}, L={:f}".format(
                            results["Lr_val"], results["Lsom_val"], results["L_val"]
                        )
                    )
            if verbose >= 2:
                print(
                    ", ".join(
                        [
                            "{}={:f}".format(metric, results[metric])
                            for metric in self.metrics
                        ]
                    )
                )
    ...
    def _compute_metrics(summary, metrics, verbose=False):
        ...
        # Internal indices
        topo_dist = summary["topological_distances"]

        # Combined error
        if "combined_error" in metrics:
            if verbose:
                print("Evaluating combined_error...")
            results["combined_error"] = combined_error(
                topo_dist, som=summary["prototypes"], d=summary["d_original"]
            )
        if "latent_combined_error" in metrics:
            if verbose:
                print("Evaluating latent_combined_error...")
            results["latent_combined_error"] = combined_error(
                topo_dist, som=summary["latent_prototypes"], d=summary["d_latent"]
            )
        if "combined_error_val" in metrics:
            if verbose:
                print("Evaluating combined_error_val...")
            results["combined_error_val"] = combined_error(
                topo_dist, som=summary["prototypes"], d=summary["d_original_val"]
            )
        if "latent_combined_error_val" in metrics:
            if verbose:
                print("Evaluating latent_combined_error_val...")
            results["latent_combined_error_val"] = combined_error(
                topo_dist, som=summary["latent_prototypes"], d=summary["d_latent_val"]
            )
        ...
        if "kruskal_shepard_error" in metrics:
            ...
            results["kruskal_shepard_error"] = kruskal_shepard_error(
                topo_dist, x=summary["X"], d=summary["d_original"]
            )
        if "latent_kruskal_shepard_error" in metrics:
            if verbose:
                print("Evaluating latent_kruskal_shepard_error...")
            results["latent_kruskal_shepard_error"] = kruskal_shepard_error(
                topo_dist, x=summary["Z"], d=summary["d_latent"]
            )
        if "kruskal_shepard_error_val" in metrics:
            if verbose:
                print("Evaluating kruskal_shepard_error_val...")
            results["kruskal_shepard_error_val"] = kruskal_shepard_error(
                topo_dist, x=summary["X_val"], d=summary["d_original_val"]
            )
        if "latent_kruskal_shepard_error_val" in metrics:
            if verbose:
                print("Evaluating latent_kruskal_shepard_error_val...")
            results["latent_kruskal_shepard_error_val"] = kruskal_shepard_error(
                topo_dist, x=summary["Z_val"], d=summary["d_latent_val"]
            )
        ...
        if "neighborhood_preservation" in metrics or "trustworthiness" in metrics:
            ...
            npr, tr = neighborhood_preservation_trustworthiness(
                1, som=summary["prototypes"], x=summary["X"], d=summary["d_original"]
            )
        ...
        if (
            "latent_neighborhood_preservation" in metrics
            or "latent_trustworthiness" in metrics
        ):
            if verbose:
                print("Evaluating latent_neighborhood_preservation_trustworthiness...")
            npr, tr = neighborhood_preservation_trustworthiness(
                1,
                som=summary["latent_prototypes"],
                x=summary["Z"],
                d=summary["d_latent"],
            )
        ...
        if (
            "neighborhood_preservation_val" in metrics
            or "trustworthiness_val" in metrics
        ):
            if verbose:
                print("Evaluating neighborhood_preservation_trustworthiness_val...")
            npr, tr = neighborhood_preservation_trustworthiness(
                1,
                som=summary["prototypes"],
                x=summary["X_val"],
        ...
        if (
            "latent_neighborhood_preservation_val" in metrics
            or "latent_trustworthiness_val" in metrics
        ):
        ...
        if "topographic_error" in metrics:
            ...
            results["topographic_error"] = topographic_error(
                topo_dist, d=summary["d_original"]
            )
        if "latent_topographic_error" in metrics:
            if verbose:
                print("Evaluating latent_topographic_error...")
            results["latent_topographic_error"] = topographic_error(
                topo_dist, d=summary["d_latent"]
            )
        if "topographic_error_val" in metrics:
            if verbose:
                print("Evaluating topographic_error_val...")
            results["topographic_error_val"] = topographic_error(
                topo_dist, d=summary["d_original_val"]
            )
        if "latent_topographic_error_val" in metrics:
            if verbose:
                print("Evaluating latent_topographic_error_val...")
            results["latent_topographic_error_val"] = topographic_error(
                topo_dist, d=summary["d_latent_val"]
            )
        ...
        if "topographic_product" in metrics:
            ...
            results["topographic_product"] = topographic_product(
                topo_dist, som=summary["prototypes"]
            )
        if "latent_topographic_product" in metrics:
            if verbose:
                print("Evaluating latent_topographic_product...")
            results["latent_topographic_product"] = topographic_product(
                topo_dist, som=summary["latent_prototypes"]
            )
        ...
        if "silhouette" in metrics:
            ...
            results["silhouette"] = skmetrics.silhouette_score(
                summary["X"], summary["y_pred"]
            )
        if "latent_silhouette" in metrics:
            if verbose:
                print("Evaluating latent_silhouette...")
            results["latent_silhouette"] = skmetrics.silhouette_score(
                summary["Z"], summary["y_pred"]
            )
        if "silhouette_val" in metrics:
            if verbose:
                print("Evaluating silhouette_val...")
            results["silhouette_val"] = skmetrics.silhouette_score(
                summary["X_val"], summary["y_val_pred"]
            )
        if "latent_silhouette_val" in metrics:
            if verbose:
                print("Evaluating latent_silhouette_val...")
            results["latent_silhouette_val"] = skmetrics.silhouette_score(
                summary["Z_val"], summary["y_val_pred"]
            )
        ...
        if "accuracy" in metrics:
            ...
            results["accuracy"] = clustering_accuracy(
                summary["y_true"], summary["y_pred"]
            )
        if "accuracy_val" in metrics:
            if verbose:
                print("Evaluating accuracy_val...")
            results["accuracy_val"] = clustering_accuracy(
                summary["y_val_true"], summary["y_val_pred"]
            )
        ...
        if "nmi" in metrics:
            ...
            results["nmi"] = skmetrics.normalized_mutual_info_score(
                summary["y_true"], summary["y_pred"]
            )
        if "nmi_val" in metrics:
            if verbose:
                print("Evaluating nmi_val...")
            results["nmi_val"] = skmetrics.normalized_mutual_info_score(
                summary["y_val_true"], summary["y_val_pred"]
            )
        ...
        if "ari" in metrics:
            ...
            results["ari"] = skmetrics.adjusted_rand_score(
                summary["y_true"], summary["y_pred"]
            )
        if "ari_val" in metrics:
            if verbose:
                print("Evaluating ari_val...")
            results["ari_val"] = skmetrics.adjusted_rand_score(
                summary["y_val_true"], summary["y_val_pred"]
            )
        ...
        if "class_scatter_index" in metrics:
            ...
            results["class_scatter_index"] = class_scatter_index(
                topo_dist, summary["y_true"], summary["y_pred"]
            )
        if "class_scatter_index_val" in metrics:
            if verbose:
                print("Evaluating csi_val...")
            results["class_scatter_index_val"] = class_scatter_index(
                topo_dist, summary["y_val_true"], summary["y_val_pred"]
            )
        ...
        if "prototype_sharpness_ratio" in metrics:
            ...
            results["prototype_sharpness_ratio"] = prototype_sharpness_ratio(
                summary["X"], summary["prototypes"]
            )
        if "prototype_sharpness_ratio_val" in metrics:
            if verbose:
                print("Evaluating prototype_sharpness_ratio_val...")
            results["prototype_sharpness_ratio_val"] = prototype_sharpness_ratio(
                summary["X_val"], summary["prototypes"]
            )
