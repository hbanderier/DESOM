...
class DESOM:
    ...
    def initialize(
        ...
        self.autoencoder, self.encoder, self.decoder = mlp_autoencoder(
            self.encoder_dims, ae_act, ae_init, batchnorm
        )
    ...
    def init_som_weights(self, X, init="random"):
        ...
        if init == "random":
            sample = X[
                np.random.choice(X.shape[0], size=self.n_prototypes, replace=False)
            ]
            encoded_sample = self.encode(sample)
            self.model.get_layer(name="SOM").set_weights([encoded_sample])
        elif init == "som":
            Z = self.encode(X)
            som = SOMNet(self.map_size[0], self.map_size[1], Z, PBC=True)
            som.train(epochs=100, start_learning_rate=0.5)
            initial_prototypes = som.weights.reshape(-1, Z.shape[1])
    ...
    def pretrain(
        self, X, optimizer="adam", epochs=200, batch_size=256, save_dir="results/tmp"
    ):
        ...
        self.autoencoder.save_weights(
            "{}/ae_weights-epoch{}.h5".format(save_dir, epochs)
        )
        print(
            "Pretrained weights are saved to {}/ae_weights-epoch{}.h5".format(
                save_dir, epochs
            )
        )
        self.pretrained = True
        ...
        while True:  # generate batches
            ...
            if X_val is not None:
                if (index_val + 1) * batch_size > X_val.shape[0]:
                    ...
                    X_val_batch = X_val[
                        index_val * batch_size : (index_val + 1) * batch_size
                    ]
                    if y_val is not None:
                        y_val_batch = y_val[
                            index_val * batch_size : (index_val + 1) * batch_size
    ...
    def fit(
        ...
        save_interval = (
            X_train.shape[0] // batch_size * save_epochs
        )  # save every save_epochs epochs
        ...
        for ite in range(iterations):
            ...
            if X_val is not None:
                _, d_val = self.model.predict(X_val_batch, verbose=0)
            ...
            h = self.neighborhood_caller(self.nodes, sigma=T)
            w_batch = h[y_pred]
            if X_val is not None:
            ...
            if ite % eval_interval == 0:
                ...
                decoded_prototypes = decoded_prototypes.reshape(
                    decoded_prototypes.shape[0], -1
                )
                # Compute pairwise squared euclidean distance matrix in original space
                d_original = np.square(
                    (
                        np.expand_dims(X_batch.reshape(X_batch.shape[0], -1), axis=1)
                        - decoded_prototypes
                    )
                ).sum(axis=2)
                if X_val is not None:
                    ...
                    d_original_val = np.square(
                        (
                            np.expand_dims(
                                X_val_batch.reshape(X_val_batch.shape[0], -1), axis=1
                            )
                            - decoded_prototypes
                        )
                    ).sum(axis=2)
                ...
                batch_summary = {
                    ...
                    "topological_distances": self.neighborhood.distances,
                    "iteration": ite,
                    "T": T,
                    "loss": loss,
        ...
        d_original = np.square(
            (
                np.expand_dims(X_train.reshape(X_train.shape[0], -1), axis=1)
                - decoded_prototypes
            )
        ).sum(axis=2)
        if X_val is not None:
            d_original_val = np.square(
                (
                    np.expand_dims(X_val.reshape(X_val.shape[0], -1), axis=1)
                    - decoded_prototypes
                )
            ).sum(axis=2)
